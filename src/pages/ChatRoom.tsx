...
const ChatRoom: React.FC = () => {
  ...
  const setupWebSocket = useCallback(async () => {
    ...
    const connectWithRetry = async (attempts = 3, delay = 5000) => {
      try {
        await websocketClient.connect(roomId, (message: Message) => {
          setMessages((prev) => [...prev, message]);
        });
        setIsConnected(true);
        toast.success("Connected to chat room");
        websocketClient.on("connect", () => setIsConnected(true));
        websocketClient.on("disconnect", () => setIsConnected(false));
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-100 to-purple-100">
      <Navbar />
      <div className="flex flex-col flex-1 mt-16 max-w-4xl mx-auto w-full">
        <div className="border-b border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700 px-6 py-4 shadow-sm rounded-t-xl">
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-gray-800 rounded-b-xl shadow-lg">
          <MessageList messages={messages} currentUser={username} />
          <ChatInput onSendMessage={handleSendMessage} isConnected={isConnected} />
        </div>
      </div>
